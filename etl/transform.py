...
def real_time_data_to_station_information(store, all_stations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for station in all_stations:
        if station['status'] == 'OPEN':
            ...
            last_station_state = store.get_one_station_last_state(station_data['number'])
            if last_station_state is None or last_station_state['last_update'] < station_data['last_update']:
